// <copyright file="MySqlTransactionDatabaseProvider.cs" company="Maomi">
...
namespace Maomi.MQ.Transaction.Mysql;

/// <summary>
/// MySQL transaction database provider.
/// </summary>
public sealed class MySqlTransactionDatabaseProvider : IDatabaseProvider, IDatabaseProviderNamed
{
    private readonly IMQTransactionOptions _options;
    private readonly string _outboxTable;
    private readonly string _inboxTable;

    /// <summary>
    /// Initializes a new instance of the <see cref="MySqlTransactionDatabaseProvider"/> class.
    /// </summary>
    /// <param name="options">Transaction options.</param>
    public MySqlTransactionDatabaseProvider(IMQTransactionOptions options)
    {
        _options = options;
    ...
    /// <inheritdoc/>
    public string ProviderName => TransactionProviderNames.MySql;
    ...
    public async Task InsertOutboxAsync(DbCommand command, OutboxMessageEntity message, CancellationToken cancellationToken = default)
    {
        ...
        AddParameter(command, "@next_retry_time", ToDbTime(message.NextRetryTime));
        AddParameter(command, "@lock_id", message.LockId ?? string.Empty);
        AddParameter(command, "@lock_time", message.LockTime.HasValue ? ToDbTime(message.LockTime.Value) : DBNull.Value);
        AddParameter(command, "@last_error", message.LastError ?? string.Empty);
        AddParameter(command, "@create_time", ToDbTime(message.CreateTime));
        AddParameter(command, "@update_time", ToDbTime(message.UpdateTime));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        AddParameter(command, "@now", ToDbTime(now));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        AddParameter(command, "@lock_id", lockId);
        AddParameter(command, "@now", ToDbTime(now));
        AddParameter(command, "@expire_time", ToDbTime(expireTime));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        AddParameter(command, "@failed", (int)MessageStatus.Failed);
        AddParameter(command, "@next_retry_time", ToDbTime(nextRetryTime));
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            AddParameter(command, "@lock_time", ToDbTime(barrier.LockTime ?? DateTimeOffset.UtcNow));
            AddParameter(command, "@last_error", barrier.LastError ?? string.Empty);
            AddParameter(command, "@create_time", ToDbTime(barrier.CreateTime));
            AddParameter(command, "@update_time", ToDbTime(barrier.UpdateTime));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        AddParameter(command, "@succeeded", (int)MessageStatus.Succeeded);
        AddParameter(command, "@cutoff_time", ToDbTime(cutoffTime));
        AddParameter(command, "@take", take);
        return await command.ExecuteNonQueryAsync(cancellationToken);
    }
    ...
    private static DateTime ToDbTime(DateTimeOffset time)
    {
