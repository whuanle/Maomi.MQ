...
public class MediatRTypeFilter : ITypeFilter
{
    ...
    public void Filter(IServiceCollection services, Type type)
    {
        ...
        IConsumerOptions? consumerAttribute = type.GetCustomAttribute<MediatRConsumerAttribute>();
        ...
        Type implementationType = typeof(MediatRConsumer<>).MakeGenericType(type);
    ...
    private static Type ValidateOrGetMiddleware(Type? eventMiddleware)
    {
        ...
        {
            return typeof(DefaultMediatrEventMiddleware<>);
        }

        if (!eventMiddleware.IsGenericTypeDefinition
            || !ImplementsOpenGenericInterface(eventMiddleware, typeof(IEventMiddleware<>)))
        {
            throw new TypeLoadException($"The type {eventMiddleware.Name} is not a valid event middleware. Example: public class DefaultMediatrEventMiddleware<TCommand> : IEventMiddleware<TCommand> {{ }}");
        }
